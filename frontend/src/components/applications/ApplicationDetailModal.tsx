...
import { FileCheck, XCircle, FileText, ExternalLink } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
      <div className="bg-slate-950 border border-slate-800 rounded-2xl max-w-lg w-full p-6 text-slate-100 shadow-2xl relative space-y-5">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-lg font-bold">Application #{appDetail.id}</h2>
            <p className="text-xs text-slate-400">
              Applied on {new Date(appDetail.created_at).toLocaleString()}
            </p>
          </div>
        </div>
